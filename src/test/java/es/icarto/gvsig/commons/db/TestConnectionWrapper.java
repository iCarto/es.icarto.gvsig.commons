...
public class TestConnectionWrapper {
	...
	public void test() {
		...
		conW.execute("CREATE TABLE " + tablename + " (gid SERIAL PRIMARY KEY, name text);");
		conW.execute("insert into " + tablename + " (name) VALUES ('foo'), ('bar');");

		DefaultTableModel t1 = conW.execute("SELECT * FROM " + tablename + " WHERE gid > 2;");
