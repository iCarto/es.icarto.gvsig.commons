...
public class ChooseLayerPanel<T extends FLayer> extends JPanel {
	...
	private final class LayerItem {
		...
		/**
		 * FLayer classes overrides toString method for some opaque reason, so we can
		 * not put it directly on a combo This is a wrapper
		 */
	...
	/**
	 * @text the text to use for the label. If null or empty Orientation.NONE will
	 *       be assumed. The text will be automatically i18n through PluginServices
	 *
	 * @param orientation if the label of the combo should be in the left of the
	 *                    combo (HORIZONTAL), over the combo (VERTICAL) or instead
	 *                    of use a label a placeholder should be used in the combo
	 *                    itself (PLACEHOLDER), or even with no label at all (NONE)
	 */
	public ChooseLayerPanel(JPanel parentComp, String text, Orientation orientation, Class<T> clasz) {
	...
	/*
	 * Preselects in the combo the first layer active of type T and returns it or
	 * null if there is no active layer
	 */
	...
	/**
	 * Adds an empty first item in the combo and preselects it [preselect] is true
	 * Take care with the order in which preselectFirstActive and this method is
	 * used
	 *
	 ...
	 *
	 * @param filter can be null to populate with all layers of the class defined in
	 *               the constructor
	 */
