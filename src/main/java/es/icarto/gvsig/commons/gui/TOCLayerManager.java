...
public class TOCLayerManager {
	...
	public <T extends FLayer> List<T> getAllLayers(Class<T> clasz) {
		...
		layers.addAll(getInnerLayers(mapControl.getMapContext().getLayers(), clasz));
		return layers;
	}
	...
	protected <T extends FLayer> List<T> getInnerLayers(FLayers layerGroup, Class<T> clasz) {
		List<T> layers = new ArrayList<T>();
