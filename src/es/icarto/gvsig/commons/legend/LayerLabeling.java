package es.icarto.gvsig.commons.legend;
...
public class LayerLabeling {
	...
	String label = null;
	if (layer.isLabeled()) {
	    final ILabelingStrategy labelingStrategy = layer
		    .getLabelingStrategy();
	...
	if (layer.getSource() instanceof VectorialFileAdapter) {
	    ...
	    String path = FileNameUtils.replaceExtension(
		    file.getAbsolutePath(), "gvlabel");
	    save(new File(path));
	} else {
	...
	if (layer.isLabeled()) {
		...
		try {
		    strategy = LabelingFactory.createStrategyFromXML(label,
			    layer);
	...
	if (labelStrategy.getLabelingMethod() != null) {
		...
		// getLabelStyle(), getTexts() and getTextSymbol() do not need
		// transformation
	    }
	}
    }
